...
function UploadMsSbomcomponent() {
  ...
  const handleScan = async (e) => {
    ...
    const response = await fetch('http://localhost:5000/MsSbomscan', {
      method: 'GET',
      headers: {
  ...
  const handlemastercsv = async (e) => {
    ...
    console.log('extract in progress')
    // alert('Extraction is in progress please wait until message shows up onscreen')
    const response = await fetch('http://localhost:5000/mastercsv', {
      method: 'GET',
      headers: {
        "Access-Control-Allow-Origin": "*",
        "Content-Type": "text/plain"
  ...
  return (
    <div>
      ...
      <h3 style={{color: "RGB(16,16,235)", textAlign:'center'}}><b>SBOM generation using Microsoft SBOM tool from Source code</b></h3>
    ...
    <a 
      href="http://localhost:5000/downloadmanifest"
      target="_blank" 
      >
    <button class="btn btn-primary" disabled={isdownloadhtmlDisabled} required>Download manifest Report</button><span data-tip="click download to download the report generated in HTML format"><AiOutlineInfoCircle /></span> <ReactTooltip />
    </a>
    <div>
      <h4><label><span style={{color: "RGB(16,16,235)"}}>Step 5:</span> Delete the SourceCode before leaving application</label></h4>
...
export default UploadMsSbomcomponent
